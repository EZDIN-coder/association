...
class Association extends Model
{
	...
	public function updateassoc($id, $id_conducteur, $id_vehicule)
		{
			$bdd = Model::getConnection();

			$sql = $bdd->prepare("UPDATE association SET id_conducteur ='".$id_conducteur."' WHERE id_association =" .$id);

			if(!$sql->execute()){
				die("ATTENTION!!!!");
			}

			header("Location: ?action=association");
		}
}
?>